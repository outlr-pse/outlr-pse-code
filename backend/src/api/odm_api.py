...
def get_all() -> Response:
    ...
    if single_odm:
        odm_one = json.load(open('mock_files/odm_one.json'))
        response = jsonify(odms=[odm_one], message="ODM successfully retrieved")
        return response

    odm_one = json.load(open('mock_files/odm_one.json'))
    odm_two = json.load(open('mock_files/odm_two.json'))
...
def get_parameters(odm_id: int) -> Response:
    ...
    hyperparams = json.load(open('mock_files/hyperparameters.json'))
